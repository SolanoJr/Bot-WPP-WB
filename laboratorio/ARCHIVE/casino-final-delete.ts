...
function safeMessageId(msg: Record<string, any>): string {
  return msg.key?.id || msg.messageId || '';
}

function safeParticipant(msg: Record<string, any>): string {
  return (msg.key?.participant || '').replace(/^@/, '') || msg.participant || '';
}

function safeRemoteJid(msg: Record<string, any>): string {
  return (msg.key?.remoteJid || '').replace(/^@/, '') || msg.remoteJid || '';
}

function safeFromMe(msg: Record<string, any>): boolean {
  return !!msg.key?.fromMe || !!msg.fromMe || false;
}

function safeSenderJid(msg: Record<string, any>): string {
