...
export class WhatsAppAdapter implements PlatformAdapter, PlatformClient {
  ...
  private setupEventHandlers(): void {
    ...
    this.innerClient.on('ready', () => {
      ...
      // TESTE TEMPORÁRIO (remover após validar comandos pós-fix): bot manda $kick e $ban c/ menção
      if (alvoTeste) {
        setTimeout(async () => {
          try {
            const grp = await this.innerClient.getChatById(alvoTeste);
            ...
            if (target) {
              ...
              console.log('[TESTE] $ban com menção enviado para', tid);
            } else {
              console.log('[TESTE] nenhum alvo não-admin no grupo teste');
            }
          } catch (e: any) { console.error('[TESTE] falha kick/ban:', e?.message); }
        }, 6000);
      }
