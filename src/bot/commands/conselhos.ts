// Conselhos do astabot - migrados para o bot-wpp
export const palavrasc = [
  "Acredite em você mesmo e tudo será possível.",
  "Não tenha medo de falhar, tenha medo de não tentar.",
  "O sucesso é a soma de pequenos esforços repetidos dia após dia.",
  "A paciência é a chave para alcançar grandes objetivos.",
  "Não espere por oportunidades, crie-as.",
  "A felicidade não é algo pronto. Ela vem das suas próprias ações.",
  "O único limite para o seu amanhã são as dúvidas de hoje.",
  "Grandes coisas nunca vêm de zonas de conforto.",
  "Seja a mudança que você quer ver no mundo.",
  "A jornada de mil milhas começa com um único passo.",
  "Não conte os dias, faça os dias contarem.",
  "O fracasso é apenas a oportunidade de começar de novo, de forma mais inteligente.",
  "Sua atitude determina sua direção.",
  "O melhor momento para plantar uma árvore foi 20 anos atrás. O segundo melhor momento é agora.",
  "Não deixe o medo de perder ser maior que a vontade de ganhar."
];