    ...
    async sendMessage(chatId, content, options = {}) {
        ...
        const sentMsg = await this.pupPage.evaluate(
            async (chatId, content, options, sendSeen) => {
                try {
                    console.log('[Client.sendMessage page.evaluate] ENTRY - chatId:', chatId, 'content:', content, 'sendSeen:', sendSeen);
                    
                    console.log('[Client.sendMessage page.evaluate] BEFORE getChat - chatId:', chatId);
                    const chat = await window.WWebJS.getChat(chatId, {
                        getAsModel: false,
                    });
                    console.log('[Client.sendMessage page.evaluate] AFTER getChat - typeof chat:', typeof chat, 'chat:', !!chat, 'chat.id:', chat?.id, 'chat.id._serialized:', chat?.id?._serialized);

                    if (!chat) {
                        console.error('[Client.sendMessage page.evaluate] CHAT IS NULL - chatId:', chatId, 'content:', content, 'options:', options);
                        return null;
                    }

                    if (sendSeen) {
                        console.log('[Client.sendMessage page.evaluate] BEFORE sendSeen - chatId:', chatId);
                        await window.WWebJS.sendSeen(chatId);
                        console.log('[Client.sendMessage page.evaluate] AFTER sendSeen');
                    }

                    console.log('[Client.sendMessage page.evaluate] BEFORE sendMessage - chat.id:', chat?.id);
                    const msg = await window.WWebJS.sendMessage(
                        chat,
                        content,
                        options,
                    );
                    console.log('[Client.sendMessage page.evaluate] AFTER sendMessage - typeof msg:', typeof msg, 'msg:', !!msg, 'msg.id:', msg?.id, 'msg.id._serialized:', msg?.id?._serialized, 'Object.keys(msg || {}):', Object.keys(msg || {}));

                    if (!msg) {
                        console.error('[Client.sendMessage page.evaluate] MSG IS NULL/UNDEFINED - chatId:', chatId, 'content:', content, 'options:', options);
                        return undefined;
                    }

                    console.log('[Client.sendMessage page.evaluate] BEFORE getMessageModel - msg.id:', msg?.id);
                    const messageModel = window.WWebJS.getMessageModel(msg);
                    console.log('[Client.sendMessage page.evaluate] AFTER getMessageModel - typeof messageModel:', typeof messageModel, 'messageModel:', !!messageModel, 'messageModel.id:', messageModel?.id, 'Object.keys(messageModel || {}):', Object.keys(messageModel || {}));
                    
                    return messageModel;
                } catch (error) {
                    console.error('[Client.sendMessage page.evaluate] ERROR:', error);
                    console.error('[Client.sendMessage page.evaluate] ERROR stack:', error.stack);
                    console.error('[Client.sendMessage page.evaluate] ERROR context - chatId:', chatId, 'content:', content, 'options:', options);
                    throw error;
                }
            },
            chatId,
        ...
        console.log('[Client.sendMessage] AFTER page.evaluate - typeof sentMsg:', typeof sentMsg, 'sentMsg:', !!sentMsg, 'Object.keys(sentMsg || {}):', Object.keys(sentMsg || {}));
