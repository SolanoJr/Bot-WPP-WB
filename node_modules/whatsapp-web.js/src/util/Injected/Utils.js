...
exports.LoadUtils = () => {
    ...
    window.WWebJS.sendMessage = async (chat, content, options = {}) => {
        ...
        // BUG 4: Msg.get() pode retornar undefined/false se mensagem ainda não indexada
        // Adicionar fallback para retornar a mensagem original enviada se Msg.get() falhar
        if (!resultMsg || resultMsg === false) {
            console.warn('[WWebJS.sendMessage] Msg.get() retornou undefined/false - usando fallback para newMsgKey');
            // Tentar construir um objeto de mensagem mínimo com newMsgKey
            const fallbackMsg = {
                id: newMsgKey,
                from: chat.id,
                to: chat.id,
                body: message?.body || '',
                timestamp: Date.now(),
                _serialized: window.WWebJS.getMsgKeyId(newMsgKey)
            };
            console.log('[WWebJS.sendMessage] FALLBACK - retornando objeto mínimo:', fallbackMsg);
            return fallbackMsg;
        }
