...
exports.LoadUtils = () => {
    ...
    // Helper function to handle _serialized -> $1 rename in WhatsApp Web 2.3000.x
    window.WWebJS.getMsgKeyId = (msgKey) => {
        if (!msgKey) return undefined;
        return msgKey._serialized || msgKey.$1 || String(msgKey);
    };
    ...
    window.WWebJS.sendMessage = async (chat, content, options = {}) => {
        ...
        // LOG DIAGNÓSTICO: Entrada da função sendMessage
        console.log('[WWebJS.sendMessage] ENTRY - chat.id:', chat?.id?._serialized, 'isChannel:', isChannel, 'isStatus:', isStatus);
        console.log('[WWebJS.sendMessage] chat.id type:', typeof chat?.id, 'isLid:', chat?.id?.isLid?.(), 'isGroup:', chat?.id?.isGroup?.(), 'isStatus:', chat?.id?.isStatus?.());
        ...
        if (isChannel) {
            ...
            console.log('[WWebJS.sendMessage] RETURN (channel) - msg.id:', msg?.id?._serialized, 'msg.serverId:', msg?.serverId);
            return msg;
        }
        ...
        if (isStatus) {
            ...
            console.log('[WWebJS.sendMessage] RETURN (status) - msg.id:', msg?.id?._serialized, 'msg.id.id:', msg?.id?.id, 'msg.body:', msg?.body?.substring(0, 50));
            return msg;
        }

        console.log('[WWebJS.sendMessage] BEFORE addAndSendMsgToChat()');
        
        const [msgPromise, sendMsgResultPromise] = window
            .require('WAWebSendMsgChatAction')
            .addAndSendMsgToChat(chat, message);
        
        console.log('[WWebJS.sendMessage] AFTER addAndSendMsgToChat() - msgPromise:', !!msgPromise, 'sendMsgResultPromise:', !!sendMsgResultPromise);
        
        await msgPromise;
        console.log('[WWebJS.sendMessage] AFTER await msgPromise()');

        if (options.waitUntilMsgSent) {
            await sendMsgResultPromise;
            console.log('[WWebJS.sendMessage] AFTER await sendMsgResultPromise()');
        }

        // LOG INSTRUMENTAÇÃO: newMsgKey antes de Msg.get()
        console.log('[WWebJS.sendMessage] BEFORE Msg.get() - newMsgKey:', newMsgKey);
        console.log('[WWebJS.sendMessage] BEFORE Msg.get() - typeof newMsgKey:', typeof newMsgKey);
        console.log('[WWebJS.sendMessage] BEFORE Msg.get() - newMsgKey._serialized:', newMsgKey?._serialized);
        console.log('[WWebJS.sendMessage] BEFORE Msg.get() - newMsgKey.$1:', newMsgKey?.$1);
        console.log('[WWebJS.sendMessage] BEFORE Msg.get() - newMsgKey.id:', newMsgKey?.id);
        
        // INVESTIGAÇÃO CHAVE: Mostrar todas as propriedades de newMsgKey
        console.log('[WWebJS.sendMessage] newMsgKey ALL PROPS:', Object.keys(newMsgKey || {}));
        console.log('[WWebJS.sendMessage] newMsgKey ALL VALUES:', Object.entries(newMsgKey || {}).map(([k, v]) => `${k}: ${typeof v} = ${v}`));
        
        const msgKeyId = window.WWebJS.getMsgKeyId(newMsgKey);
        console.log('[WWebJS.sendMessage] BEFORE Msg.get() - getMsgKeyId(newMsgKey):', msgKeyId);
        
        // INVESTIGAÇÃO MsgStore: Tipo e estrutura
        const MsgStore = window.require('WAWebCollections').Msg;
        console.log('[WWebJS.sendMessage] MsgStore type:', typeof MsgStore);
        console.log('[WWebJS.sendMessage] MsgStore constructor:', MsgStore?.constructor?.name);
        console.log('[WWebJS.sendMessage] MsgStore has .get:', typeof MsgStore.get);
        console.log('[WWebJS.sendMessage] MsgStore has .has:', typeof MsgStore.has);
        console.log('[WWebJS.sendMessage] MsgStore has .set:', typeof MsgStore.set);
        console.log('[WWebJS.sendMessage] MsgStore size:', MsgStore.size);
        
        // INVESTIGAÇÃO: Listar algumas chaves existentes no MsgStore para entender o formato
        try {
            const sampleKeys = [];
            let count = 0;
            if (MsgStore._map) {
                MsgStore._map.forEach((value, key) => {
                    if (count < 5) {
                        sampleKeys.push({ key, type: typeof key, id: value?.id?._serialized || value?.id?.$1 });
                        count++;
                    }
                });
            }
            console.log('[WWebJS.sendMessage] MsgStore sample keys:', sampleKeys);
        } catch (e) {
            console.log('[WWebJS.sendMessage] Could not sample MsgStore keys:', e.message);
        }
        
        // INVESTIGAÇÃO 3: Polling do Store.Msg em diferentes tempos
        const checkTimes = [0, 50, 100, 250, 500, 1000, 2000];
        
        for (const delay of checkTimes) {
            if (delay > 0) {
                await new Promise(resolve => setTimeout(resolve, delay));
            }
            const checkResult = MsgStore.get(msgKeyId);
            const checkExists = MsgStore.has ? MsgStore.has(msgKeyId) : !!checkResult;
            console.log(`[WWebJS.sendMessage] POLLING Store.Msg.get() at ${delay}ms - exists: ${checkExists}, result:`, checkResult ? 'FOUND' : 'NOT_FOUND', 'type:', typeof checkResult);
        }
        
        const resultMsg = MsgStore.get(msgKeyId);
        
        // LOG INSTRUMENTAÇÃO: Resultado de Msg.get()
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg:', resultMsg);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - typeof resultMsg:', typeof resultMsg);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg === false:', resultMsg === false);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg === undefined:', resultMsg === undefined);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg === null:', resultMsg === null);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg?.id:', resultMsg?.id);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg?.id._serialized:', resultMsg?.id?._serialized);
        console.log('[WWebJS.sendMessage] AFTER Msg.get() - resultMsg?.id.$1:', resultMsg?.id?.$1);
        
        // REMOVIDO: Fallback temporário para investigar causa raiz
        // if (!resultMsg || resultMsg === false) { ... }
        
        // LOG INSTRUMENTAÇÃO: Antes de getMessageModel()
        console.log('[WWebJS.sendMessage] BEFORE getMessageModel() - resultMsg:', !!resultMsg);
        
        const messageModel = window.WWebJS.getMessageModel(resultMsg);
        
        // LOG INSTRUMENTAÇÃO: Resultado de getMessageModel()
        console.log('[WWebJS.sendMessage] AFTER getMessageModel() - messageModel:', !!messageModel);
        console.log('[WWebJS.sendMessage] AFTER getMessageModel() - typeof messageModel:', typeof messageModel);
        console.log('[WWebJS.sendMessage] AFTER getMessageModel() - messageModel.id:', messageModel?.id);
        console.log('[WWebJS.sendMessage] AFTER getMessageModel() - messageModel.id._serialized:', messageModel?.id?._serialized);
        console.log('[WWebJS.sendMessage] AFTER getMessageModel() - messageModel.id.$1:', messageModel?.id?.$1);
        
        console.log('[WWebJS.sendMessage] RETURN - retornando messageModel');
        return messageModel;
    };
    ...
    window.WWebJS.getMessageModel = (message) => {
        ...
        if (typeof msg.id.remote === 'object') {
            msg.id = Object.assign({}, msg.id, {
                remote: window.WWebJS.getMsgKeyId(msg.id.remote),
            });
        }
    ...
    window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
        ...
        if (chat.groupMetadata) {
            ...
            const chatWid = window
                .require('WAWebWidFactory')
                .createWid(window.WWebJS.getMsgKeyId(chat.id));
            const groupMetadata =
        ...
        if (model.msgs && model.msgs.length) {
            const lastMessage = chat.lastReceivedKey
                ? window
                      .require('WAWebCollections')
                      .Msg.get(window.WWebJS.getMsgKeyId(chat.lastReceivedKey)) ||
                  (
                      await window
                          .require('WAWebCollections')
                          .Msg.getMessagesById([
